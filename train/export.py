...
def write_mfcc_tables(out_dir=WEIGHTS):
    ...
    body = "".join([
        ...
        f"#define MFCC_MEL_SHIFT 21\n#define MFCC_LOG10_2_Q29 {int(quant.round_away(10.0 * np.log10(2.0) * (1 << 29)))}\n",
        ...
        c_array("int32_t", "mfcc_log2_lut", quant.round_away(np.log2(1.0 + np.arange(257) / 256.0) * 65536).astype(np.int64)),
    ])
...
def write_c_headers(model, ckpt, q, out_dir=WEIGHTS):
    ...
    for f in layers:
        ...
        raw += [c_array("float", f"{n}_raw_w", f["w"]), c_array("float", f"{n}_bn_scale", f["bn_scale"]),
                c_array("float", f"{n}_bn_shift", f["bn_shift"])]
    table = ", ".join(f"{{{f['name']}_w, {f['name']}_b}}" for f in layers)
    (out_dir / "weights_f32.h").write_text(c_header("WEIGHTS_F32", "".join(['#include "forward.h"\n\n'] + fused + [
    ...
    (out_dir / "weights_raw.h").write_text(c_header("WEIGHTS_RAW", "".join(['#include "forward.h"\n\n'] + raw + [
    ...
    (out_dir / "weights_int8.h").write_text(c_header("WEIGHTS_INT8", "".join(['#include "forward.h"\n\n'] + body + fc_arrays + [
